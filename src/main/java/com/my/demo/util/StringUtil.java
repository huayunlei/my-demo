...
public class StringUtil {
    ...
    private final static Pattern htmlScriptPattern = Pattern.compile("<script[^>]*?>[\\s\\S]*?<\\/script>", Pattern
            .CASE_INSENSITIVE);

    //定义style的正则表达式: <[\\s]*?style[^>]*?>[\\s\\S]*?<[\\s]*?\\/[\\s]*?style[\\s]*?>
    private final static Pattern htmlStylePattern = Pattern.compile("<style[^>]*?>[\\s\\S]*?<\\/style>", Pattern
            .CASE_INSENSITIVE);
    ...
    public static String getRandomAlphabetString(int strLen) {
...
*/
    ...
    public static String toHex(byte byteData[], int offset, int len) {
        ...
        for (int i = offset; i < len; i++) {
            ...
            buf[k++] = digits[((int) byteData[i] & 0x0f)];
        }
...
*/
    ...
    public static boolean isJsonObject(String s) {
        if (isEmpty(s))
            return false;
        if ((s.startsWith("{")) || (s.startsWith("[")) || (s.equals("true")) || (s.equals("false")) || (s.equals
                ("null"))) {
            return true;
        }
        return isNumber(s);
    ...
    public static boolean isMobilePhone(String s) {
        if (isEmpty(s))
            return false;
        return Pattern.matches("^(13|14|15|17|18)\\d{9}$", s);
    }
    ...
    public static String bytes2Guid(byte[] buf, int offset) {
        ...
        StringBuffer sb = new StringBuffer();
        sb.append(hex.subSequence(6, 8)).append(hex.subSequence(4, 6)).append(hex.subSequence(2, 4)).append(hex
                .subSequence(0, 2));
